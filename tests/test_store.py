"""Test JSON file store — messages, channels, notes, and logs."""
...
async def test_persistence_across_instances(tmp_path):
    ...
    s1 = Store(data_dir)
    await s1.init()
    chat_id = await s1.create_channel("persistent")
...
async def test_backward_compat_missing_fields(tmp_path):
    """Old channel files may lack notes/logs fields."""
    data_dir = str(tmp_path / "data")
    store = Store(data_dir)
    await store.init()

    # Write a minimal old-format file
