...
#
# This copies the daemon script + runtime helpers, installs uv, and sets up a venv with dependencies.
# After deploying:
...
ssh "$REMOTE" "mkdir -p $REMOTE_DIR/dcc_runtime"
scp tools/orchestrator_daemon.py "$REMOTE:$REMOTE_DIR/orchestrator_daemon.py"
scp tools/dcc_runtime/*.py "$REMOTE:$REMOTE_DIR/dcc_runtime/"
...
ssh "$REMOTE" "source \$HOME/.local/bin/env 2>/dev/null; uv venv $REMOTE_DIR/.venv && uv pip install --python $REMOTE_DIR/.venv/bin/python3 claude-agent-sdk aiohttp mcp"

echo ""
echo "Done. Next steps on remote:"
echo "  1. Install/auth the agent runtime you plan to use on the server:"
echo "     - Claude backend: Claude Code CLI + auth"
echo "     - Codex backend: Codex CLI + auth (ensure 'codex app-server' is on PATH)"
echo ""
echo "  2. Start daemon (once per server, e.g. in tmux):"
